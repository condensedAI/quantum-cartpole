...
def control_random(env):
    ...
    env.difference = np.abs(((env.estimate - env.measurement_mean/env.L) / ( env.max_position * 2 ) ) )
...
class QuantumCartPoleEnvV0(gym.Env):
    ...
    def __init__(self, N_meas = 1, k = np.pi, system = 'quantum', potential = 'quadratic', controller = 'rlc', estimator = 'rle', state_return = 'state_running',filter_model = None):
        ...
        #Array defining the classical linear system, used in classical simulation/LQR//KalmanFilter
        ...
        #Selecting the appropriate functions depending on the input system
    ...
    def step(self, action):
        ...
        # Termination of a run. self.reward and self.run_count are used in training to gain additional statistics 
        if np.abs(self.actual_pos[0]) > self.max_position or self.number_of_steps_taken >= self.termination:
            self.rewards += reward
            self.run_count += 1
